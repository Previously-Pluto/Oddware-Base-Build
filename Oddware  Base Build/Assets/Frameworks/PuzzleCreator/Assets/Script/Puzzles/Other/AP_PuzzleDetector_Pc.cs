...
public class AP_PuzzleDetector_Pc : MonoBehaviour {
    ...
    {
        Debug.Log("Camera " + objMainCamera.transform.GetChild(0).gameObject.name);
        #region
    ...
    private IEnumerator WaitAFewSeconds()
    {
        ...
        #endregion
        //AP_GlobalPuzzleManager_Pc.instance.currentPuzzle.transform.parent.gameObject.SetActive(false);
        accessPuzzle.GetComponent<actionsWhenPuzzleIsSolved_Pc>().m_onPuzzleCompleted?.Invoke();
        AP_GlobalPuzzleManager_Pc.instance.currentPuzzle = null;
        AP_GlobalPuzzleManager_Pc.instance.m_currentPuzzle.gameObject.SetActive(false);
        AP_GlobalPuzzleManager_Pc.instance.ActivateNovel();
    }
