...
namespace SlimUI.CursorControllerPro {
    ...
    public class CursorController : PointerInputModule {
        ...
        [Tooltip("If enabled, the game will start with input detection turned ON.")]
        public bool startEnabled = true;
        [Tooltip("If enabled, both Desktop and Controller input will use the animated cursors and never the default operating system cursors.")]
        public bool alwaysUseAnimatedCursor = false;
        ...
        public void SetAlwaysOnCursors()
        {
            GetComponent<EventSystem>().enabled = true;
        ...
        private void SelectedPlayerMovement(){ // Moves active player with the mouse & gets cursor boundaries
            ...
            if(selfCanvas.renderMode == RenderMode.ScreenSpaceOverlay){
                ...
                mousePosition = cameraMain.ScreenToWorldPoint(inputProvider.GetMousePosition());
            }
        ...
        private void SetMovementToCurrentPlayerInput(){
            if (currentPlayerActive == 0){ // only detect movement for active cursor, ignore the others
                xMovement = Time.deltaTime * (generalSettingsInstance.horizontalSpeed * 100) * inputProvider.GetAxis(horizontalAxis);
                yMovement = Time.deltaTime * (generalSettingsInstance.verticalSpeed * 100) * inputProvider.GetAxis(verticalAxis);
            }else if (currentPlayerActive == 1 && player2Exists){
                xMovement = Time.deltaTime * (generalSettingsInstance.horizontalSpeed * 100) * inputProvider.GetAxis(horizontalAxis2);
                yMovement = Time.deltaTime * (generalSettingsInstance.verticalSpeed * 100) * inputProvider.GetAxis(verticalAxis2);
            }else if (currentPlayerActive == 2 && player3Exists){
                xMovement = Time.deltaTime * (generalSettingsInstance.horizontalSpeed * 100) * inputProvider.GetAxis(horizontalAxis3);
                yMovement = Time.deltaTime * (generalSettingsInstance.verticalSpeed * 100) * inputProvider.GetAxis(verticalAxis3);
            }else if (currentPlayerActive == 3 && player4Exists){
                xMovement = Time.deltaTime * (generalSettingsInstance.horizontalSpeed * 100) * inputProvider.GetAxis(horizontalAxis4);
                yMovement = Time.deltaTime * (generalSettingsInstance.verticalSpeed * 100) * inputProvider.GetAxis(verticalAxis4);
            }
        }
            ...
            if(canMoveCursor){
                switch( m_State ){
                    case eInputState.MouseKeyboard:
                        if(inputProvider.IsGamepadActive(currentXAxis, currentYAxis, generalSettingsInstance.deadZone)){
                    ...
                    case eInputState.Controller:
                        if (inputProvider.IsMouseKeyboard()){
