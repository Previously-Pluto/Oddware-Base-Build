//Description : SlotMachine_PC : Manage the slot game behaviour
#if (UNITY_EDITOR)
...
[CustomEditor(typeof(CustomUIPuzzle_PC))]
public class CustomUIPuzzleEditor_PC : Editor
{
    ...
    SerializedProperty customUIGameType;
    ...
    void OnEnable()
    {
        ...
        CustomUIPuzzle_PC myScript = (CustomUIPuzzle_PC)target;


        customUIGameType = serializedObject.FindProperty("customUIGameType");
    ...
    private void DisplayPuzzleCreationSection(CustomUIPuzzle_PC myScript, GUIStyle style_blue)
    {
        #region
        EditorGUILayout.BeginVertical(style_blue);
        EditorGUILayout.HelpBox("(1) PRESS THE BUTTON: the player must press the button before it disapears \n (2) NUMBER: the player presses the button in order \n (3) COLORS: the player presses the corresponding color", MessageType.Info);
        _helpBox(0);

        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.LabelField("CUSTOM UI GAME TYPE :", GUILayout.Width(220));
        EditorGUILayout.PropertyField(customUIGameType, new GUIContent(""), GUILayout.Width(300));
        EditorGUILayout.EndHorizontal();
    ...
    private void DisplayNaniNovelSection(CustomUIPuzzle_PC myScript, GUIStyle style_blue)
    {
        #region
        EditorGUILayout.BeginVertical(style_blue);
        _helpBox(3);
    ...
    private void GenerateKeys(CustomUIPuzzle_PC myScript)
    {
        #region
    ...
    private void otherSection(CustomUIPuzzle_PC myScript, GUIStyle style_Orange)
    {
