...
namespace SlimUI.CursorControllerPro
{
    public class TooltipController : MonoBehaviour
    {
        ...
        public void ToolTipPopUpDelay()
        {
            ...
            {
                if (cursorController.currentPlayerActive == 0)
                    tooltipRect.GetComponent<Animator>().SetBool("Show", true);
                else if (cursorController.currentPlayerActive == 1)
                    tooltipRect2.GetComponent<Animator>().SetBool("Show", true);
                else if (cursorController.currentPlayerActive == 2)
                    tooltipRect3.GetComponent<Animator>().SetBool("Show", true);
                else if (cursorController.currentPlayerActive == 3)
                    tooltipRect4.GetComponent<Animator>().SetBool("Show", true);
                countTimer = false;
            }
        }
        ...
        {
            if (cursorController.currentPlayerActive == 0)
                tooltipRect.transform.localPosition = Vector3.SmoothDamp(tooltipRect.transform.localPosition,
                    toolTipPosition, ref toolTipV, toolTipSmoothing);
            if (cursorController.currentPlayerActive == 1)
                tooltipRect2.transform.localPosition = Vector3.SmoothDamp(tooltipRect2.transform.localPosition,
                    toolTipPosition, ref toolTipV, toolTipSmoothing);
            if (cursorController.currentPlayerActive == 2)
                tooltipRect3.transform.localPosition = Vector3.SmoothDamp(tooltipRect3.transform.localPosition,
                    toolTipPosition, ref toolTipV, toolTipSmoothing);
            if (cursorController.currentPlayerActive == 3)
                tooltipRect4.transform.localPosition = Vector3.SmoothDamp(tooltipRect4.transform.localPosition,
                    toolTipPosition, ref toolTipV, toolTipSmoothing);
        }

        public void ToolTipBoundaries(RectTransform rect)
        {
            if (rect.anchoredPosition.x <= cursorController.xMin + (toolTipWidth + 70))
            {
                // Too Far left
                tooFarLeft = true;
            ...
            if (rect.anchoredPosition.x >= cursorController.xMax - (toolTipWidth + 70))
            {
                // Too Far Right
                tooFarRight = true;
                if (rect.anchoredPosition.y <= cursorController.yMin + (toolTipHeight + 55))
        ...
        {
            if (cursorController.currentPlayerActive == 0) tooltipTitle1.text = title;
            tooltipBody1.text = body;
            if (cursorController.currentPlayerActive == 1) tooltipTitle2.text = title;
            tooltipBody2.text = body;
            if (cursorController.currentPlayerActive == 2) tooltipTitle3.text = title;
            tooltipBody3.text = body;
            if (cursorController.currentPlayerActive == 3) tooltipTitle4.text = title;
            tooltipBody4.text = body;
        }
        ...
        {
            if (tooltipRect.GetComponent<Animator>().runtimeAnimatorController != null)
