...
[CustomEditor(typeof(SlotMachine_PC))]
public class SlotMachineEditor_PC : Editor
{
    ...
    public string[] toolbarStrings = new string[] { "Game Options"};
    SerializedProperty aP_PuzzleDetector;

    SerializedProperty aP_PuzzleMoveType;

    SerializedProperty b_PuzzleSolved;         // Know if the puzzle is solved

    SerializedProperty _conditionsToAccessThePuzzle;    // access conditionsToAccessThePuzzle component
    SerializedProperty _actionsWhenPuzzleIsSolved;      // access actionsWhenPuzzleIsSolved component
    ...
    void OnEnable()
    {
        ...
        SlotMachine_PC myScript = (SlotMachine_PC)target;
        ...
        else
        {
            ...
            if (tilesList.arraySize > 0)
            {
                ...
                if (b_TilesExist)
                {
                    // --> Display option
                    if (toolbarCurrentValue.intValue == 0)
                        otherSection(myScript, style_Blue);
                }
            }
        }
    ...
    private void otherSection(SlotMachine_PC myScript, GUIStyle style_Orange)
    {
