    ...
    public static class TransformTools
    {
        ...
        private static void PlaceOnSurface(Transform target, PlaceOnSurfaceData data, MeshFilter[] filters)
        {
            ...
            foreach (var vertexTransform in dirVert)
            {
                ...
                if(data.placeOnColliders)
                {
                    if (!Physics.Raycast(rayOrigin, worldProjDir, out hitInfo, float.MaxValue, data.mask)) continue;
                }
                else
                {
                    if (!RaycastMesh.Raycast(rayOrigin, worldProjDir, out hitInfo, out MeshFilter colliderFilter, filters, float.MaxValue)) continue;
                }
        ...
        public static void PlaceOnSurface(GameObject[] selection, PlaceOnSurfaceData data)
        {
            ...
            foreach (var obj in selection)
            {
                PlaceOnSurface(obj.transform, data, filters);
            }
        ...
        public class SimulateGravityData
        {
            ...
            private float _maxAngularSpeed = 10;
        ...
        public static void SimulateGravity(GameObject[] selection, SimulateGravityData data)
        {
            ...
            foreach (var obj in selection)
            {
                UnityEditor.Undo.RecordObject(obj.transform, "Simulate Gravity");
