	...
	public override void OnInspectorGUI ()
	{
		...
		else
		{
			...
			for( int i = 0; i < targ.RadialMenuStyles.Count; i++ )
			{
				...
				GUIStyle assignedSpritesStyle = new GUIStyle( EditorStyles.label ) { alignment = TextAnchor.MiddleRight, onActive = new GUIStyleState() { textColor = Color.black }, richText = true };
				GUILayout.Label( assignedSpritesIndication, assignedSpritesStyle );
