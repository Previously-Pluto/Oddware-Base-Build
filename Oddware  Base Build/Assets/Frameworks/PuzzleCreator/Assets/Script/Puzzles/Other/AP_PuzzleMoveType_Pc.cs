// AP_PuzzleMoveType_Pc: This script is used on puzzles except Gear and Logic puzzles
using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class AP_PuzzleMoveType_Pc : MonoBehaviour
{
    public bool             SeeInspector = false;
    public int              puzzleMoveMode = 0;
                                                ...
                                                Camera cameraUseForFocus)
    {
        ...
        else if (puzzleMoveMode == 1)
        {
            ...
            if (_detectClick.DetectPuzzleStateObjectsRaycast(myLayer, false))
            {
                if (!b_OnlyOneTimeNoSelection)
                {
                    b_OnlyOneTimeSelected = false;
                    b_OnlyOneTimeNoSelection = true;
                    aP_GlobalPuzzle.aP_DragAndDropParent.AP_CanGrabLogicOrGearMethods();
                }
            }
            else
            {
                if (!b_OnlyOneTimeSelected)
                {
                    ...
                    aP_GlobalPuzzle.aP_DragAndDropParent.AP_LogicOrGearNoSelectionMethods();
                }
            }
        }
        //VR Hand
                                  ...
                                  )
    {
        ...
        if (objClicked != null && objClicked.transform.name == "btnPuzzleReset" && 
            !b_PuzzleSolved && 
            objClicked.transform.parent.parent.gameObject == gameObject) // Player press button reset 
        {
            //F_ResetPuzzle();
            ...
            objClicked.transform.name == "ExitPuzzle" && 
            objClicked.transform.parent.parent.gameObject == gameObject) // Player press button Exit puzzle 
        {
            if (aP_PuzzleDetector.b_FocusActivated)
            {
            ...
            && objClicked.transform.name == "Clue" && 
            objClicked.transform.parent.parent.gameObject == gameObject) // Player press button Clue
        {
    ...
    {
        #region
        if (newComponent.GetType() == typeof(PipesPuzzle_Pc))
            puzzle.GetComponent<PipesPuzzle_Pc>().F_ResetPuzzle();

        if (newComponent.GetType() == typeof(LeverPuzzle_Pc))
            puzzle.GetComponent<LeverPuzzle_Pc>().F_ResetPuzzle();

        if (newComponent.GetType() == typeof(SlidingPuzzle_Pc))
            puzzle.GetComponent<SlidingPuzzle_Pc>().F_ResetPuzzle();

        if (newComponent.GetType() == typeof(cylinderPuzzle_Pc))
            puzzle.GetComponent<cylinderPuzzle_Pc>().F_ResetPuzzle();

        if (newComponent.GetType() == typeof(DigicodePuzzle_Pc))
            puzzle.GetComponent<DigicodePuzzle_Pc>().F_ResetPuzzle();

        if (newComponent.GetType() == typeof(TypingGamePuzzle_Pc))
            puzzle.GetComponent<TypingGamePuzzle_Pc>().F_ResetPuzzle();
        #endregion
    }
}
